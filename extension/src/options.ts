...
async function test(){statusEl.className='status';statusEl.textContent='Checking native host…';try{const r=await chrome.runtime.sendMessage({type:'health'});statusEl.textContent=r?.ok?'Rivlet is connected.':r?.error||'Not connected';statusEl.classList.add(r?.ok?'ok':'error');}catch(e){statusEl.textContent=String(e);statusEl.classList.add('error');}}
