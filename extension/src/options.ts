...
const statusEl=$<HTMLDivElement>('status'),video=$<HTMLInputElement>('video-enabled'),sites=$<HTMLTextAreaElement>('disabled-sites'),captureDownloads=$<HTMLInputElement>('capture-downloads');
async function refresh(){const saved=await chrome.storage.local.get(['videoEnabled','disabledSites','captureDownloads']);video.checked=Boolean(saved.videoEnabled);captureDownloads.checked=saved.captureDownloads!==false;sites.value=(saved.disabledSites||[]).join('\n');}
...
$<HTMLButtonElement>('save').onclick=async()=>{await chrome.storage.local.set({videoEnabled:video.checked,captureDownloads:captureDownloads.checked,disabledSites:sites.value.split(/\r?\n/).map(x=>x.trim().toLowerCase()).filter(Boolean)});};
