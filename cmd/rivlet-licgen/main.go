// Command rivlet-licgen is a developer tool for the licensing system. It
...
//	go run ./cmd/rivlet-licgen keygen
//	export RIVLET_LICENSE_PUBKEY=<public key printed above>   # (set in the app's environment)
//
//	# 2. Mint a Pro certificate bound to a device id (see the app's License page):
//	go run ./cmd/rivlet-licgen sign -priv <private key> -device dev-abc123 -license LIC-TEST-0001
//
...
func usage() {
	fmt.Fprintln(os.Stderr, "usage: rivlet-licgen <keygen|sign> [flags]")
...
func keygen() {
	...
	fmt.Printf("PUBLIC  (RIVLET_LICENSE_PUBKEY / bake into keys.go): %s\n", base64.StdEncoding.EncodeToString(pub))
...
func sign(args []string) {
	...
	product := fs.String("product", "rivlet-pro-lifetime", "product id")
